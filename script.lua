...
Window:AddSlider({ Title = "Slider", Description = "Sliding", Tab = Main, MaxValue = 100,
	Callback = function(Amount) warn(Amount) end })

Window:AddToggle({ Title = "Toggle", Description = "Switching", Tab = Main,
	Callback = function(Boolean) warn(Boolean) end })

Window:AddInput({ Title = "Input", Description = "Typing", Tab = Main,
	Callback = function(Text) warn(Text) end })
...
Window:AddDropdown({
	...
	Callback = function(Number) warn(Number) end,
})

Window:AddKeybind({ Title = "Keybind", Description = "Binding", Tab = Main,
	Callback = function(Key) warn("Key Set") end })
...
Window:AddKeybind({
	...
	Tab = Settings,
	Callback = function(Key) Window:SetSetting("Keybind", Key) end,
})
...
Window:AddDropdown({
	...
	Callback = function(Theme) Window:SetTheme(Themes[Theme]) end,
})
...
Window:AddToggle({
	...
	Default = true,
	Tab = Settings,
	Callback = function(Boolean) Window:SetSetting("Blur", Boolean) end,
})
...
UserInputService.InputBegan:Connect(function(Key)
	if Key == Keybind then warn("You have pressed the minimize keybind!") end
end)
